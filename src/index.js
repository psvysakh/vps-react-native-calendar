...
const DatePicker = ({
  ...
  const dayPress = (day) => {
    if (!startDate) {
      ...
      setMarkedDates({
        [day.dateString]: {marked: true, color: "#e8e8e8", textColor: "black"},
      });
      rangeSelected({startDate:day.dateString});
    } else if (startDate && !endDate) {
      // Selecting end date
      setEndDate(day.dateString);
      let range = getRange(startDate, day.dateString);
      setMarkedDates(range);
      rangeSelected({marked: true,startDate:startDate,endDate:day.dateString});
    } else {
      // Both dates already selected
      setStartDate(day.dateString);
      setEndDate(null);
      setMarkedDates({
        [day.dateString]: { selected: true, selectedColor: "#14a800" },
      });
    }
  };
    ...
    const getRange = (start, end) => {
      ...
      while (currentDate <= stopDate) {
        ...
        if(start==dateString){
          range[dateString] = { startingDay: true, color: '#14a800', textColor: 'white'};
        }
        if(end==dateString){
          range[dateString] = { endingDay: true, color: '#14a800', textColor: 'white'};
          return range;
        }
        if(start!==dateString){
          range[dateString] = { color: "#e8e8e8", textColor: "black" };
        }
...
const styles = StyleSheet.create({
  ...
  monthYear: (isSelected, isDisabled, selectedStyles) => ({
    backgroundColor: isDisabled || !isSelected ? "#ffffff" : "#14a800",
    borderRadius: 10,
